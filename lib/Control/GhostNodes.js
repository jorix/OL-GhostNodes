...
OpenLayers.Control.GhostNodes = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function(){
        ...
        var clearIndexes = function() {
            _self.indexFeatures = indexFeatures = {};
            _self.indexNodes = indexNodes = {};
        };
        var clearIindexModified = function() {
            _self.indexModified = indexModified = {};
        };
        ...
        var addToIndex = function(features) {
            for (var i = 0, len = features.length; i < len; i++) {
                var feature = features[i],
                    line = _self.getLineString(feature);
                if (line) {
     ...
     */
    completeModification: function(layer, feature, modified) {
        if (!feature.modified) {
