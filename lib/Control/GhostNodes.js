/* Copyright 2012-2017 by Xavier Mamano, http://github.com/jorix/OL-GhostNodes
...
OpenLayers.Control.GhostNodes = OpenLayers.Class(OpenLayers.Control, {
        ...
        var addToIndex = function(features) {
            for (var i = 0, len = features.length; i < len; i++) {
                var feature = features[i],
                    line = this.getValidLineString(feature);
                if (line) {
        ...
        var eachModification = function(functionItem) {
            ...
            for (var i = 0, len = layers.length; i < len; i++) {
                ...
                for (var featureId in indexOriginalGeometry) {
                    var item = indexFeatures[featureId];
                    if (item) {
                        ...
                        if (feature.state !== OpenLayers.State.DELETE) {
                            ...
                            removeFromIndex([feature]);
                            delete indexOriginalGeometry[featureId];
                        }
                    } else {
                        delete indexOriginalGeometry[featureId];
                    }
    ...
    onVertexModified: function(evt) {
        ...
        var feature = evt.feature,
            line = this.getValidLineString(evt.feature);
        if (line) {
            var nodes = null,
                vertex = evt.vertex,
    ...
    modifyVertex: function(newVertex, vertexType, feature) {
        var line = this.getValidLineString(feature);
        if (line) {
     ...
     */
    getValidLineString: function(feature) {
        if (!feature) {
            1;
        }
